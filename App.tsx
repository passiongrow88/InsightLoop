...
function App() {
  ...
  useEffect(() => {
    ...
    if (userDataStr) {
      try {
        ...
        // Prefer user saved language if exists inside data, otherwise keep global/current
  ...
  // 3. Save User Data on Change (Debounced slightly by React nature)
  useEffect(() => {
  ...
  const handleUpdateUser = (updatedUser: User) => {
    ...
    // Also update the main user registry in localstorage (so it persists across re-logins)
  ...
  if (!isAppReady) return null; // Or a loading spinner
